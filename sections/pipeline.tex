...
Stage~3 is a Python emitter that turns the artifacts of Stage~2 into a complete TAIDL specification of the form shown in Listing~\ref{lst:taidl-example}.
It consumes two parallel inputs: the \texttt{taidl.*}-annotated MLIR produced by Pass~D8, and auxiliary RTL analysis that recovers per-instruction state updates and, for CISC-style loop macros, the primitive instruction vocabulary and loop-bound configuration registers.

\paragraph{Primitive instruction assembly.}
The assembler operates on a closed vocabulary of thirteen \texttt{taidl.*} attribute keys.
After parsing each annotated function into a Python record, it merges the per-(instruction, ASV) functions emitted by Stage~1 back into per-instruction groups using the instruction names from the same descriptor that drove Stage~1, falling back to a longest-prefix heuristic when no descriptor is available---so the same code path handles Gemmini's four-instruction \texttt{ExecuteController} and VTA's single-instruction modules without modification.
Within each group the assembler selects the metadata record with the richest dataflow as a template and dispatches its \texttt{tensor\_op} through two parallel paths.
The \emph{compute path} maps each tensor op to its XLA-HLO template: \texttt{mac}/\texttt{dot\_product} expand to \texttt{convert + dot + add} (with optional \texttt{clamp}), \texttt{pool} to \texttt{reduce(max)}, and \texttt{im2col\_matmul} to \texttt{reshape + dot + add}.
The \emph{DMA path} is driven by \texttt{taidl.dma\_role} tags attached to target-ASV functions whose ports belong to a DMA request bundle (e.g., \texttt{io\_dma\_req\_bits\_vaddr} $\to$ \texttt{dma\_dram\_addr}, \texttt{io\_dma\_req\_bits\_laddr\_data} $\to$ \texttt{dma\_spad\_addr}); the assembler collects the role set across the instruction group and emits a \texttt{load} or \texttt{store} body accordingly, distinguishing \texttt{mvin} from \texttt{mvout} without any per-instruction routing logic.
Each emitted instruction body follows the v2 TAIDL IDL format consumed by ACT's ANTLR-based parser: operands enter as flat \texttt{u8} buffers that are reshaped and bitcast-converted to signed types, the core tensor operation is expressed using HLO-level primitives (\texttt{dot}, \texttt{add}, \texttt{reshape}, \texttt{reduce}, etc.), and the result is narrowed and bitcast back to \texttt{u8}---matching the canonical form that JAX emits for integer workloads and that ACT's pattern matcher expects.
Instruction attributes (shapes, addresses, mode flags) are derived from the register-field roles supplied by the same instruction descriptor.

\paragraph{CISC macro composition.}
Real workloads on accelerators like Gemmini are typically dispatched not as individual \texttt{compute\_preloaded} calls but through CISC-style loop macros (\texttt{loop\_ws}, \texttt{loop\_conv\_ws}) that orchestrate a sequence of primitive instructions behind a single driver invocation.
TensorLift handles these through two RTL-parsing helpers that are independent of the lifting pipeline: one statically parses the arbiter wiring in the loop controller's flattened Verilog to recover the set of primitive funct codes the macro can issue; the other extracts per-funct state-register writes from the controller's dispatch tables.
The assembler composes the primitive vocabulary's tensor operation (e.g., the \texttt{dot\_product} from the compute sub-FSM) over the extracted loop bounds (from the config companions' state updates) into a whole-program HLO body.
The matmul-vs-convolution distinction is currently resolved by a \texttt{macro\_type} tag in the instruction descriptor (one token per CISC macro); we note that it could alternatively be inferred from the dimensionality of the iteration space, since convolution loops carry spatial-dimension bounds that matrix-multiply loops do not.
For instructions whose RTL encodes an activation field, the assembler emits a variant (e.g., \texttt{loop\_ws\_relu}) that inserts a \texttt{maximum(result, broadcast(0))} before the narrowing chain, with the activation-to-HLO mapping derived from the extracted register value.

\paragraph{Inferring the instruction state machine.}
The constraints and updates that gate primitive instructions---e.g., that \texttt{compute\_preloaded} can fire only after \texttt{preload} has armed the controller---are not visible in the Stage~2 output, which is per-instruction by construction.
For primitive instructions whose extraction directories contain LLVM IR files, we recover constraints with a script that reads the per-ASV update functions in their LLVM IR form, where the \texttt{icmp eq} comparisons against control registers that Pass~A1 collapses are still intact.
The script auto-detects the FSM register by scoring all multi-writer scalar ASVs against three predicates: small bit-width ($\leq\!8$), the presence of \texttt{icmp eq} comparisons at the register's own bit-width (which filters out boolean event flags), and at least two distinct compared constants.
Once the FSM register is fixed, each instruction's constraint is the set of constants its update functions compare the register against, with pipeline-flush boundary values filtered as boilerplate; instructions that write the FSM but test no explicit value are handled by a binary-FSM complement rule.
For the newer CISC loop directories (which contain only \texttt{.tmp.mlir} and no \texttt{.ll}), state updates are instead obtained by the RTL dispatch-table parser described above, which extracts per-funct register writes directly from the Verilog \texttt{always} blocks.
In both cases the recovered state information is folded into the emitted TAIDL as structured metadata comments rather than runtime-callable API methods, consistent with the v2 TAIDL format.

On the running Gemmini example, this Stage~3 reproduces the structure of the hand-written reference TAIDL specification: the four primitive ExecuteController instructions carry the same constraints and state-update relations as the reference, the DMA-role synthesis correctly distinguishes \texttt{mvin} from \texttt{mvout}, and the CISC composition produces whole-program \texttt{loop\_ws} and \texttt{loop\_conv\_ws} bodies that match the reference's semantics.
The only difference is that the inferred FSM register retains its RTL name (\texttt{control\_state}) where the hand-written specification uses a TAIDL-level alias (\texttt{preload}).
As we report in \S\ref{sec:evaluation}, the test oracles produced from TensorLift's specification are character-identical to those produced from the hand-written reference, because the oracle generator alpha-renames state registers consistently across both inputs.
